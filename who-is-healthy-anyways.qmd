...
plot_racial_gap_change <- function(refs_adult_list, refs_youth_list, single_sex = FALSE){
  ...
  # Select functions based on single_sex parameter
  if(single_sex) {
    race_estimate_fev1_fn <- race_estimate_fev1_single_sex
    race_estimate_fvc_fn <- race_estimate_fvc_single_sex
  } else {
    race_estimate_fev1_fn <- race_estimate_fev1
    race_estimate_fvc_fn <- race_estimate_fvc
  }
  ...
  race_coeffs_fev1_adult <- map_df(refs_adult_list, race_estimate_fev1_fn) %>% 
    mutate(sequence=sequence_adult_df$ref,
           Measure = "FEV1") 
  
  race_coeffs_fev1_youth <- map_df(refs_youth_list, race_estimate_fev1_fn) %>% 
    mutate(sequence=sequence_youth_df$ref,
           Measure = "FEV1") 
  
  race_coeffs_fvc_adult <- map_df(refs_adult_list, race_estimate_fvc_fn) %>% 
    mutate(sequence=sequence_adult_df$ref,
           Measure = "FVC")
  
  race_coeffs_fvc_youth <- map_df(refs_youth_list, race_estimate_fvc_fn) %>% 
    mutate(sequence=sequence_youth_df$ref,
           Measure = "FVC")
