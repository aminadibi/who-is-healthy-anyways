...
We used data from three cycles of NHANES (2007-2008, 2009-2010, and 2011-2012), which was designed to be representative of the non-institutionalized civilian population of the US. Our study population included children aged 6-19 and adults aged 20-80. We included individuals with baseline spirometry that either met or exceeded ATS collection standards.
...
#   print(df)
    ...
    df %>% 
      ...
      summarise(change = 100*(last(estimate) - first(estimate)) / first(estimate)) %>% 
      round(2) 
      } else if (type=="abs"){
        df %>% 
        filter(Measure == measure) %>%
        summarise(change = -1*(last(estimate)  - first(estimate)),
                  low    = -1*(last(conf.low)  - first(conf.low)),
                  high   = -1*(last(conf.high) - first(conf.high)) %>% round(3)) 
      } else if (type=="baseline") {
        df %>% 
        filter(Measure == measure) %>%
        summarise(change = -1*(first(estimate)),
                  low    = -1*(first(conf.low)),
                  high   = -1*(first(conf.high)) %>% round(3)) 
      } else if (type=="final") {
        df %>% 
        filter(Measure == measure) %>%
        summarise(change = -1*(last(estimate)),
                  low    = -1*(last(conf.low)),
                  high   = -1*(last(conf.high)) %>% round(3)) 
...
plot_racial_gap_change <- function(refs_adult_list, refs_youth_list, single_sex = FALSE, 
  ...
  if (dfOutput) {
    if (outputType=="perc") {
        df_adult_reduction_metric1 <- as.numeric(compute_gap_reduction(race_coeffs_adult, metric1, type="perc")$change)
        df_youth_reduction_metric1 <- as.numeric(compute_gap_reduction(race_coeffs_youth, metric1, type="perc")$change)
        df_adult_reduction_metric2 <- as.numeric(compute_gap_reduction(race_coeffs_adult, metric2, type="perc")$change)
        df_youth_reduction_metric2 <- as.numeric(compute_gap_reduction(race_coeffs_youth, metric2, type="perc")$change)}
    
    if (outputType=="abs") {
        df_adult_reduction_metric1 <- as.numeric(compute_gap_reduction(race_coeffs_adult, metric1, type="abs")$change)
        df_youth_reduction_metric1 <- as.numeric(compute_gap_reduction(race_coeffs_youth, metric1, type="abs")$change)
        df_adult_reduction_metric2 <- as.numeric(compute_gap_reduction(race_coeffs_adult, metric2, type="abs")$change)
        df_youth_reduction_metric2 <- as.numeric(compute_gap_reduction(race_coeffs_youth, metric2, type="abs")$change)}
...
message("Adult counts:")
message(map_df(refs_adult_1by1, count))

refs_youth_1by1 <- createCohort("youth", leaveOneIn = TRUE)
message("Youth counts:")
message(map_df(refs_youth_1by1, count))
...
### Two and Three Criteria Analysis
...
exclude_all_combinations <- function(exclusionList=c("homeowner", 
  ...
  for (pair in exclusionPairs) {
    ...
    if (n==1){
    refs_adult <- createCohortFlex("adult", exclusions = c(exclusion1))
    refs_youth <- createCohortFlex("youth", exclusions = c(exclusion1))
    }
    if (n==2){
    refs_adult <- createCohortFlex("adult", exclusions = c(exclusion1, exclusion2))
    refs_youth <- createCohortFlex("youth", exclusions = c(exclusion1, exclusion2))
    } 
    if (n==3) {
      refs_adult <- createCohortFlex("adult", exclusions = c(exclusion1, exclusion2, exclusion3))
      refs_youth <- createCohortFlex("youth", exclusions = c(exclusion1, exclusion2, exclusion3))
    }
...
ones <- exclude_all_combinations(n=1)

pairs <- exclude_all_combinations(n=2)

triplets <- exclude_all_combinations(n=3)

quartets <- exclude_all_combinations(n=4)
```
