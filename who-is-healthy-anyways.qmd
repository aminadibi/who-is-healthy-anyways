...
plot_racial_gap_change <- function(refs_adult_list, refs_youth_list, single_sex = FALSE){
  ...
  race_coeffs_fev1_adult <- 
    map_df(refs_adult_list, function(x) race_estimate(x, "fev1", single_sex = single_sex)) %>% 
    mutate(sequence=sequence_adult_df$ref,
           Measure = "FEV1") 
  
  race_coeffs_fev1_youth <-
    map_df(refs_youth_list, function(x) race_estimate(x, "fev1", single_sex = single_sex)) %>%
    mutate(sequence=sequence_youth_df$ref,
           Measure = "FEV1") 
  
  race_coeffs_fvc_adult <- 
    map_df(refs_adult_list, function(x) race_estimate(x, "fvc", single_sex = single_sex)) %>%
    mutate(sequence=sequence_adult_df$ref,
           Measure = "FVC")
  
  race_coeffs_fvc_youth <- 
    map_df(refs_youth_list, function(x) race_estimate(x, "fvc", single_sex = single_sex)) %>% 
    mutate(sequence=sequence_youth_df$ref,
           Measure = "FVC")
