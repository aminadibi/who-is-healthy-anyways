...
ggsave("figure4.pdf", figure4, width=10, height=8, units="in", dpi=300)
...
```{r}
library(mice)
createCohortImpute <- function (ageGroup, incomePovertyRatioOnly=FALSE, sex='all', BirthCountry='all', homeOwnershipOnly=FALSE, includedRace=c(3,4), AllostaticLoadOnly=FALSE, leaveOneIn=FALSE) {
  if (!(ageGroup %in% c("adult", "children", "all_ages"))) 
    stop("Invalid age group")
  if (!(BirthCountry %in% c("all", "US", "Other countries"))) 
    stop("Invalid BirthCountry")
  ...
  nhanes_raw <- readRDS("nhanes_2007_2012.rds") 
  # %>%
  #   select(SEQN, MEC6YR, age, sex, SDMVPSU, SDMVSTRA, race, race_text_nhanes,
  #          fev1_pre, fvc_pre, spiro_status_pre, height, fev1_quality_pre, fvc_quality_pre,
  #          ever_100_smoke, current_smoke, last_5_days_cigarettes, last_5_days_pipes, last_5_days_cigars, ever_asthma, ever_bronchitis, ever_emphysema, cough_3_month, phlegm_3_month, wheezing_past_yr, dry_cough_night_past_yr, mineral_dusts, exhaust_fumes, organic_dusts, other_fumes, vigorous_work, vigorous_recreational, moderate_work,  moderate_recreational, maternal_smoke, smoker_in_household, bmi, home_ownership, has_insurance, education_adults, now_attending_school, healthy_diet
  #          )
  
  # Select variables you want to impute (adjust as needed)
  impute_vars <- c("age", "sex", "race", "height",
                   "home_ownership", "vigorous_work", "vigorous_recreational",
                   "moderate_work", "moderate_recreational", "smoker_in_household", 
                   "mineral_dusts", "organic_dusts","exhaust_fumes", "other_fumes",
                   "bmi", "has_insurance")

  # Run mice
  imputed_data <- mice(nhanes_raw[, impute_vars], m = 1, method = 'pmm', maxit = 5)

  # Replace original data with imputed values
  nhanes_raw[, impute_vars] <- mean(complete(imputed_data, 1))


    # Run mice
  # imputed_data <- mice(nhanes_raw, m = 1, method = 'pmm', maxit = 5)
  # 
  # # Replace original data with imputed values
  # nhanes_raw <- complete(imputed_data, 1)
...
```{r}
refs_adult <- createCohortImpute("adult")
message("Adult counts:")
message(map_df(refs_adult, ~ count(.x$variables)))

refs_youth <- createCohortImpute("children")
...
plot_racial_gap_change(refs_adult, refs_youth) + plot_annotation(
  caption = "Non-Hispanic Black vs. Non-Hispanic White population - Imputed Dataset"
  )

plot_racial_gap_change(refs_adult, refs_youth, dfOutput=TRUE, outputType="perc") 
...
# Step 1: Extract data
nhanes_mice <- nhanes_included$variables %>%
  select(age,
         race_text_nhanes,
         sex,
         mineral_dusts,
...
# Step 2: Check missingness visually (optional)
vis_miss(nhanes_mice)

# Step 3: Run multiple imputation (m = number of datasets)
imp <- mice(nhanes_mice, m = 5, method = 'pmm', seed = 123)
...
createCohortCompleteCase <- function (ageGroup, incomePovertyRatioOnly=FALSE, sex='all', BirthCountry='all', homeOwnershipOnly=FALSE, includedRace=c(3,4), AllostaticLoadOnly=FALSE, leaveOneIn=FALSE) {
  if (!(ageGroup %in% c("adult", "children", "all_ages"))) 
    stop("Invalid age group")
  if (!(BirthCountry %in% c("all", "US", "Other countries"))) 
    stop("Invalid BirthCountry")
...
ref1 <- ref0 %>%
  srvyr::filter (((current_smoke == "3")) &
         ((ever_100_smoke != "1")) &
         ( (last_5_days_cigarettes != "1")) &
         ((last_5_days_pipes != "1")) &
         ((last_5_days_cigars != "1")) &
         ((ever_asthma !="1")) &
         ((ever_bronchitis != "1")) &
         ((ever_emphysema != "1")) &
         ((cough_3_month!=1)) & 
         ((phlegm_3_month!=1)) & 
         ((wheezing_past_yr!=1)) & 
         ((dry_cough_night_past_yr!=1))) %>%
  mutate(ref="reference")


if (!incomePovertyRatioOnly & !homeOwnershipOnly & !AllostaticLoadOnly &!leaveOneIn) {
ref2 <- ref1 %>%
    srvyr::filter(((mineral_dusts!=1)) & 
         ((organic_dusts!=1)) & 
         ((exhaust_fumes!=1)) & 
         ((other_fumes!=1))) %>%
  mutate(ref="+ job unexposed")
...
ref3 <- ref2 %>%
      srvyr::filter(((vigorous_work==1)) | 
         ((vigorous_recreational==1)) | 
         ((moderate_work==1)) | 
         ((moderate_recreational==1)))  %>%
  mutate(ref="+ physically active")

ref4 <- ref3 %>%
      srvyr::filter(((maternal_smoke!=1))) %>% #TODO Maternal smoke not available for those over 15 years
  mutate(ref="+ no maternal smoke")
  
ref5 <- ref4 %>%  
      srvyr::filter(((smoker_in_household != "1"))) %>%
      mutate(ref="+ no 2ndhand smoke")
...
if (ageGroup=="adult") {
  ref9 <- ref8 %>%
  srvyr::filter((education_adults %in% c(4,5)))  %>%
  mutate(ref="+ educated")
}

if (ageGroup=="children") {
  ref9 <- ref8 %>%
    filter((now_attending_school != 3)| (age<6))  %>%
  mutate(ref="+ educated")
}
...
refs_adult_cc <- createCohortCompleteCase("adult")
message("Adult counts:")
message(map_df(refs_adult, ~ count(.x$variables)))

refs_youth_cc <- createCohortCompleteCase("children")
message("Youth counts:")
message(map_df(refs_youth, ~ count(.x$variables)))

plot_racial_gap_change(refs_adult_cc, refs_youth_cc) + plot_annotation(
  caption = "Non-Hispanic Black vs. Non-Hispanic White population - Imputed Dataset"
  )

plot_racial_gap_change(refs_adult_cc, refs_youth_cc, dfOutput=TRUE, outputType="perc") 
```
